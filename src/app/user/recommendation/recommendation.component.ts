...
export class RecommendationComponent implements OnInit {
  ...
  this.loading = true; // 💡 Affiche la barre de progression AVANT l'appel
  ...
  this.expenseService.recommendationStreamFetch(
    ...
    next: (text) => {
      if (this.loading) {
        this.loading = false; // 💥 Cache la barre au premier caractère reçu
      }
    ...
    error: (err) => {
      ...
      this.loading = false; // Cacher aussi en cas d'erreur
    },
    complete: () => {
      this.loading = false; // Cacher si jamais terminé sans erreur
    }
  });
}
